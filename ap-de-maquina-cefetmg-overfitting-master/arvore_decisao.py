...
def cria_modelo(X:pd.DataFrame ,y:pd.Series ,min_samples_split:float) -> DecisionTreeClassifier:
    """
...
def faz_classificacao(x_treino:pd.DataFrame, y_treino:pd.Series, x_teste:pd.DataFrame, y_teste:pd.Series, min_samples_split:float) -> Tuple[List[float],float]:
    ...
    #calcule a acurácia
    acuracia = np.sum([True for y_p,y_t in zip(y_predicted,y_teste) if y_p == y_t ])/len(y_teste)
